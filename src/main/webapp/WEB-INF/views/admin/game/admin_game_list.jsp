		...
		<div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4">		
			<div class="row">						
				...
				<div class="col-sm-12 col-md-6">
					<div class="dataTables_length" style="text-align: center;">
						<label>
							<c:choose>
								<c:when test="${search.g_result == '2' }">
									<button type="button" class="btn btn-primary resultChk" value="1">경기 전</button>&nbsp;	
									<button type="button" class="btn btn-primary resultChk" value="2" disabled="disabled">경기 후</button>								
								</c:when>
								<c:when test="${search.g_result == '1' }">
									<button type="button" class="btn btn-primary resultChk" value="1" disabled="disabled">경기 전</button>&nbsp;	
									<button type="button" class="btn btn-primary resultChk" value="2">경기 후</button>
								</c:when>
								<c:otherwise>
									<button type="button" class="btn btn-primary resultChk" value="1">경기 전</button>&nbsp;	
									<button type="button" class="btn btn-primary resultChk" value="2">경기 후</button>								
						...
						<label>
				 			...
				 			<c:choose>
				 				<c:when test="${empty search.category or search.category =='선택' }">
									...
									<option value="NAME_STADIUM">경기장</option>
				 				</c:when>
				 				<c:otherwise>
				 					<c:choose>
					 					<c:when test="${search.category eq 'CATEGORY' }">
						 					<option>선택</option>
											<option value="CATEGORY" selected="selected">종목</option>
											<option value="TITLE">제목</option>
											<option value="CONTENT">내용</option>
											<option value="NAME_STADIUM">경기장</option>	 						
					 					</c:when>
					 					...
					 					<c:when test="${search.category eq 'NAME_STADIUM' }">
						 					<option>선택</option>
											<option value="ID">작성자</option>
											<option value="TITLE">제목</option>
											<option value="CONTENT">내용</option>
											<option value="NAME_STADIUM" selected="selected">경기장</option>	 						
					 					</c:when>					 					
				 					</c:choose>	 						 					
				 				</c:otherwise>
				 			</c:choose>
			...
			<div class="row">		
				<div class="col-sm-12">
					...
					<colgroup><col width="5%"><col width="5%"><col width="10%"><col width="10%"><col width="15%"><col width="15%"><col width="20%"><col width="20%"></colgroup>			
					<thead>
						<tr><th><input type="checkbox" id="_checkAll" style="height: 15px; width: 15px; margin-bottom: 5px;"></th><th>번호</th><th>종목</th><th>경기장</th><th>1번팀</th><th>2번팀</th><th>제목</th><th>경기 일정</th></tr>
					</thead>
					<tbody>
						<c:choose>
							...
							<c:otherwise>
								<c:forEach	var="i" items="${list }" varStatus="vs">
									<tr class="notice_list_tr" seq_game=${i.seq_game }>
									<th onclick="event.cancelBubble=true" class="checkBoxs"><input type="checkbox" style="height: 15px; width: 15px; margin-top: 5px;" class="delCheck" seq_game=${i.seq_game }></th>
									<th>${totalCount - ((pDto.curPage-1)*10+(vs.count-1)) }</th>
									<th>${i.category }</th>
									<th>${i.name_stadium }</th>
									<th>${i.name_team1 }</th>
									<th>${i.name_team2 }</th>
									<th>
									<c:choose>
										<c:when test="${fn:length(i.title) gt 40 }">
											${fn:substring(i.title,0,39) }...
										</c:when>
										<c:otherwise>
											${i.title }
										</c:otherwise>
									...
									<th>${fn:substring(i.gdate,0,10) }</th>
									</tr>
								</c:forEach>					
							</c:otherwise>
						</c:choose>
...
<form action="#" id="_frmForm">
	...
	<input type="hidden" name="g_result" value="${search.g_result }"> 
</form>
<script src="/admin-js/admin-gameList.js"></script>
