...
<thead>
	<tr>
		<td colspan="3" style="padding: 2px;">
			<input type="image" src="../image/search2.png" style="width: 23px; height: 23px; margin-left: 5px; margin-top: 5px; float: right;" onclick="search4()">
			<input type="text" name="keyword" class="form-control" placeholder="TEAM 검색" maxlength="20" style="width: 130px; float: right;">
		</td>
	</tr>
	<tr>
		<th colspan="3">BADMINTON <img alt="" src="../image/badminton.png" style="width: 22px; height: 22px;"></th>
	</tr>
	<tr>
...
<tbody>
	<c:if test="${empty ranklist4 }">
	<tr>
		<td colspan="3" style="text-align: center;">랭킹 목록이 없습니다.</td>
	</tr>
	</c:if>
	
	<c:forEach items="${ranklist4 }" var="rank" varStatus="vs">	<!-- 시작 : 1 -->
		...
		<c:choose>
			...
			<td>${rank.badminton_mmr }</td>
		</tr>
	
	</c:forEach>
	
	<tr align="center" class="trfoot">
		<td colspan="3" id="paging_td" style="border-bottom: 0;">
		<ul class="pagination pagination-sm">
		<c:if test="${gparam.prev }">
			<li class="page-item"><a class="page-link" href="MMRRankList4.do?keyword=${gparam.keyword }&pageNum=${gparam.startPage-1}">prev</a></li>
		</c:if>
		...
		<c:forEach begin="${gparam.startPage }" end="${gparam.endPage }" step="1" var="index">
			...
			<c:if test="${gparam.pageNum ne index }">
				<li class="page-item"><a class="page-link" href="MMRRankList4.do?keyword=${gparam.keyword }&pageNum=${index}">${index}</a></li>
				<%-- <li class="page-item"><a class="page-link" onclick="page(${index})">${index}</a></li> --%>
			</c:if>
		</c:forEach>
		
		<c:if test="${gparam.next }">
			<li class="page-item"><a class="page-link" href="MMRRankList4.do?keyword=${gparam.keyword }&pageNum=${gparam.endPage+1}">next</a></li>
		</c:if>
		</ul>
		</td>
	</tr>
...
function search4() {
	$("#_frm4").attr("action", "MMRRankList4.do").submit();
}
