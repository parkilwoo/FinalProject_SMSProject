...
<thead>
	<tr>
		<td colspan="3" style="padding: 2px;">
			<input type="image" src="../image/search2.png" style="width: 23px; height: 23px; margin-left: 5px; margin-top: 5px; float: right;" onclick="search2()">
			<input type="text" name="keyword" class="form-control" placeholder="TEAM 검색" maxlength="20" style="width: 130px; float: right;">
		</td>
	</tr>
	<tr>
		<th colspan="3">BASKETBALL <img alt="" src="../image/basketball.png" style="width: 22px; height: 22px;"></th>
	</tr>
	<tr>
...
<tbody>
	<c:if test="${empty ranklist2 }">
	<tr>
		<td colspan="3" style="text-align: center;">랭킹 목록이 없습니다.</td>
	</tr>
	</c:if>
	
	<c:forEach items="${ranklist2 }" var="rank" varStatus="vs">	<!-- 시작 : 1 -->
	...
	<tr align="center" class="trfoot">
		...
		<c:if test="${gparam.prev }">
			<li class="page-item"><a class="page-link" href="MMRRankList2.do?keyword=${gparam.keyword }&pageNum=${gparam.startPage-1}">prev</a></li>
		</c:if>
		...
		<c:forEach begin="${gparam.startPage }" end="${gparam.endPage }" step="1" var="index">
			...
			<c:if test="${gparam.pageNum ne index }">
				<li class="page-item"><a class="page-link" href="MMRRankList2.do?keyword=${gparam.keyword }&pageNum=${index}">${index}</a></li>
			</c:if>
		</c:forEach>
		
		<c:if test="${gparam.next }">
			<li class="page-item"><a class="page-link" href="MMRRankList2.do?keyword=${gparam.keyword }&pageNum=${gparam.endPage+1}">next</a></li>
		</c:if>
		</ul>
		</td>
	</tr>
...
function search2() {
	$("#_frm2").attr("action", "MMRRankList2.do").submit();
}
